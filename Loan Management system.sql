...
-- Criteria Based Income
start transaction;
Create Table Grade_Wise
select *,if(ApplicantIncome > 15000 , " Grade A " ,
		 if(ApplicantIncome > 9000 , " Grade B " , 
         if(ApplicantIncome > 5000 , " Middle Class ", " Low Class ")))
         as Criteria_Based_Income from Customer_income;
commit;
select * from Grade_Wise;

-- Monthly Interest Percentage
start transaction;
drop table monthly_interest_percentage;
Create Table Monthly_Interest_Percentage
select *,if(ApplicantIncome<5000 and Property_area ="Rural" , " 3" ,
		 if(ApplicantIncome<5000 and Property_area ="SemiRural" , " 3.5" ,
         if(ApplicantIncome<5000 and Property_area ="Urban" , " 5" ,
         if(ApplicantIncome<5000 and Property_area ="SemiUrban" , " 2.5" , " 7"))))
         as Monthly_Interest_Percentage from Customer_income;
commit;
select * from Monthly_Interest_Percentage;


-- Primary table 
	create table Loan_det (Loan_id varchar(55) , customer_id varchar(55) , Loan_Amount varchar(55) ,
	Loan_amount_term int , cibil_score int , primary key(Loan_id));
        
delimiter //
	create trigger Loan_amt before insert on Loan_det for each row
		begin
...
-- Secondary Table
	create table Remarks_update_det (Loan_id varchar(55) , Loan_amount varchar(55) , 
	Cibil_score int , Cibil_Status varchar(55) , primary key(Loan_id));
        
                                                                                                                       
delimiter //
create trigger Cibil_score after insert on Loan_det for each row
begin
	if new.cibil_score > 900 then 
        insert into remarks_update_det(Loan_id, Loan_amount , Cibil_score , Cibil_status) values 
	...
	elseif new.cibil_score > 0 then 
        insert into remarks_update_det(Loan_id, Loan_amount , Cibil_score , Cibil_status) values 
        (new.Loan_id , new.Loan_amount , new.cibil_score ,"Penalty Customer");
        
	elseif new.cibil_score <= 0 then 
        insert into remarks_update_det(Loan_id, Loan_amount , Cibil_score , Cibil_status) values 
        (new.Loan_id , new.Loan_amount , new.cibil_score ,"Reject Customer");
...
select * from remarks_update_det;        
select * from monthly_interest_percentage;

start transaction;

delete from loan_det where Loan_Amount = "Loan Still Processing";


commit;

Alter Table loan_det modify Loan_Amount int;

commit;

-- joining the table Loan Details And Monthly Interest percentage

create Table Customer_Interest_Analysis
select M.Loan_id , M.Customer_id , M.ApplicantIncome ,M.CoApplicantIncome ,M.Property_Area,
M.Loan_status , L.Loan_Amount , L.Loan_Amount_Term , L.Cibil_score ,
concat(M.Monthly_Interest_Percentage," %") as Monthly_Interest_Percentage ,
round(L.Loan_amount/100*M.Monthly_interest_percentage,2) as Monthly_Interest ,
round(((L.Loan_amount/100)*M.Monthly_interest_percentage)*12,2) as Annual_Interest
from Monthly_interest_percentage M inner join Loan_Det L on M.loan_id = L.loan_id;
...
delete from customer_det where customer_id ="IP43007";
...
-- Join all the 5 tables without repeating the fields - output 1
 ...
 select CD.Customer_ID,CD.Customer_name,CD.Gender,CD.Age,CD.Married,CD.Education,
		CD.Self_Employed,LD.Loan_Id,LD.Loan_Amount,LD.Loan_amount_term,
		LD.cibil_score,CI.ApplicantIncome,CI.CoapplicantIncome,CI.Property_Area,
        CI.Loan_Status,CS.Postal_Code,CS.Segment,CS.State,RI.Region_Id,RI.Region 
        from customer_det CD inner join Loan_det LD on CD.Customer_id = LD.Customer_id
        inner join Customer_income CI on CD.Customer_id = CI.Customer_id inner join 
        Country_state CS on CS.Customer_id = CD.Customer_id inner join Region_Info RI 
        on CD.Region_id = RI.Region_Id;

-- *find the mismatch details using joins - output 2 

create table Mismatch_det
select RI.Region_Id,RI.Region,CD.Customer_ID,CD.Customer_name,
CD.Gender,CD.Age,CD.Married,CD.Education,CD.Self_Employed
from Region_info RI left Join Customer_Det CD on RI.Region_id = CD.Region_id;

-- Filter high cibil score - output 3
create table high_cibil_score
select * from remarks_update_det where Cibil_Status ="High cibil Score";
        
--  Filter home office and corporate - output 4

create Table Home_Corporate
select CD.Customer_ID,CD.Customer_name,CD.Gender,CD.Age,CD.Married,
	   CD.Education,CD.Self_Employed,LD.Loan_Id,LD.Loan_Amount,
       LD.Loan_amount_term,LD.cibil_score,CI.ApplicantIncome,
       CI.CoapplicantIncome,CI.Property_Area,CI.Loan_Status,
	   CS.Postal_Code,CS.Segment,CS.State,RI.Region_Id,RI.Region from 
       customer_det CD inner join Loan_det LD on CD.Customer_id = LD.Customer_id 
	   inner join Customer_income CI on CD.Customer_id = CI.Customer_id 
	   inner join Country_state CS on CS.Customer_id = CD.Customer_id
	   inner join Region_Info RI on CD.Region_id = RI.Region_Id where 
       segment in("Home office","Corporate");


 -- All Outputs Store to procedure        
 
Delimiter //
Create procedure Loan_management_system()
begin
select * from All_Table;
select * from Mismatch_det;
select * from High_cibil_score;
select * from Home_Corporate;
end //
Delimiter ;

-- Call procedure

Call loan_management_system();
